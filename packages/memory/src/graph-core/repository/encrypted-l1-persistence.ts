...
import type { V4GraphPersistence } from '../adapters/v4-graph-memory'

const ALGORITHM = 'aes-256-gcm'
const SNAPSHOT_SCHEMA = 'continuum-memory-graph-l1'
const KEY_SCHEMA = 'continuum-memory-graph-l1-key'

interface EncryptedL1Envelope {
  readonly version: 1
...
interface ProtectedL1KeyEnvelope {
  readonly version: 1
  readonly schema: typeof KEY_SCHEMA
  readonly protectedKey: string
}

export interface EncryptedGraphL1PersistenceOptions {
...
/** Separate authenticated L1 checkpoint; deliberately does not reuse V4 files or keys. */
export function createEncryptedGraphL1Persistence(options: EncryptedGraphL1PersistenceOptions): V4GraphPersistence {
  function existingKey(): Buffer {
    if (!existsSync(options.keyPath))
      throw new Error('Protected L1 key does not exist')
    const envelope = JSON.parse(readFileSync(options.keyPath, 'utf8')) as ProtectedL1KeyEnvelope
    if (envelope.version !== 1 || envelope.schema !== KEY_SCHEMA || typeof envelope.protectedKey !== 'string')
      throw new Error('Invalid protected L1 key envelope')
  ...
  return {
    ...
    save(payload: string): void {
      if (options.readOnly)
        throw new Error('L1 persistence is read-only')
      const key = getOrCreateKey()
      const encrypted = encrypt(payload, key)
      if (decrypt(encrypted, key) !== payload)
        throw new Error('L1 encryption verification failed')
...
function encrypt(payload: string, key: Buffer): string {
  ...
  } satisfies EncryptedL1Envelope)
}

function decrypt(payload: string, key: Buffer): string {
  assertKey(key)
  const envelope = JSON.parse(payload) as EncryptedL1Envelope
  if (envelope.version !== 1 || envelope.schema !== SNAPSHOT_SCHEMA || envelope.algorithm !== ALGORITHM
    ...
    throw new Error('Invalid encrypted L1 envelope')
  try {
    ...
    throw new Error('Unable to authenticate encrypted L1 snapshot')
  }
}

function assertKey(key: Buffer): void {
  if (!Buffer.isBuffer(key) || key.length !== 32)
    throw new Error('L1 master key must be exactly 32 bytes')
}
