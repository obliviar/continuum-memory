...
$testRoot = Join-Path ([IO.Path]::GetTempPath()) ("continuum-memory-v4-smoke-{0}" -f [guid]::NewGuid().ToString('N'))
...
try {
  $env:CONTINUUM_MEMORY_USER_DATA_DIR = $dataPath
  $env:CONTINUUM_MEMORY_BOOT_LOG = $logPath
  $env:CONTINUUM_MEMORY_SMOKE_TEST = 'true'
  $env:CONTINUUM_MEMORY_ENABLED = 'true'
  $env:CONTINUUM_MEMORY_V4_INTERNAL_REVIEW = 'true'
  $env:CONTINUUM_MEMORY_V4_READ_MODE = 'auto'
  $env:CONTINUUM_MEMORY_SMOKE_EXPECT_ROLLOUT_STAGE = 'internal'
  ...
  Remove-Item Env:CONTINUUM_MEMORY_SMOKE_EXPECT_ROLLOUT_STAGE -ErrorAction SilentlyContinue
  ...
  $env:CONTINUUM_MEMORY_SMOKE_PURGE_ID = 'smoke-manual-name'
  Invoke-SmokeLaunch
  Remove-Item Env:CONTINUUM_MEMORY_SMOKE_PURGE_ID -ErrorAction SilentlyContinue
  ...
  Remove-Item Env:CONTINUUM_MEMORY_USER_DATA_DIR,Env:CONTINUUM_MEMORY_BOOT_LOG,Env:CONTINUUM_MEMORY_SMOKE_TEST,Env:CONTINUUM_MEMORY_SMOKE_PURGE_ID,Env:CONTINUUM_MEMORY_SMOKE_EXPECT_ROLLOUT_STAGE,Env:CONTINUUM_MEMORY_ENABLED,Env:CONTINUUM_MEMORY_V4_INTERNAL_REVIEW,Env:CONTINUUM_MEMORY_V4_READ_MODE -ErrorAction SilentlyContinue
  if (Test-Path -LiteralPath $testRoot) {
    $resolved = [IO.Path]::GetFullPath($testRoot)
    $temporaryRoot = [IO.Path]::GetFullPath([IO.Path]::GetTempPath()).TrimEnd('\') + '\'
    $name = [IO.Path]::GetFileName($resolved)
    $insideTemporaryRoot = $resolved.StartsWith($temporaryRoot, [StringComparison]::OrdinalIgnoreCase)
    $hasExpectedName = $name.StartsWith('continuum-memory-v4-smoke-')
