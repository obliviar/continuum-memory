...
import { createEncryptedGraphRelationPersistence } from './encrypted-relation-persistence'

describe('encrypted L2 relation persistence', () => {
  let directory: string
  beforeEach(() => { directory = mkdtempSync(join(tmpdir(), 'graph-relation-test-')) })
  afterEach(() => { rmSync(directory, { recursive: true, force: true }) })

  function persistence(readOnly = false) {
    return createEncryptedGraphRelationPersistence({
